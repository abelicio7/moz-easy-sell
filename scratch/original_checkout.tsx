...
const Checkout = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.payment_method === "mpesa" && !["84", "85"].includes(prefix)) {
      toast.error("Para pagar com M-Pesa, o n├║mero deve come├ºar com 84 ou 85.");
      return;
    }
    
    if (form.payment_method === "emola" && !["86", "87"].includes(prefix)) {
      toast.error("Para pagar com E-Mola, o n├║mero deve come├ºar com 86 ou 87.");
      return;
    }
    ...
    try {
      ...
      console.log("Iniciando cria├º├úo do pedido:", orderId);
      ...
      // --- SUPABASE REALTIME: Escuta a confirma├º├úo do pagamento ---
      ...
      // Limpeza de seguran├ºa (timeout de 2 minutos)
      setTimeout(() => {
        supabase.removeChannel(channel);
      }, 120000);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4 text-center">
        ...
        <p className="text-muted-foreground max-w-xs mx-auto">
          Este produto n├úo est├í mais dispon├¡vel para compra ou ainda est├í aguardando aprova├º├úo.
        </p>
              ...
              <CardContent className="pt-8 space-y-8">
                  ...
                  <div className="mt-6 space-y-4">
                    <div className="space-y-1.5">
                      <Label className="text-xs text-muted-foreground flex items-center justify-between uppercase tracking-wider font-bold">
                        <span className="flex items-center gap-1 font-bold text-foreground">N├║mero de Pagamento *</span>
                      ...
                      <p className="text-[10px] text-muted-foreground italic text-center">
                        Ir├ís receber um pedido de confirma├º├úo (PIN) neste n├║mero via {form.payment_method === "mpesa" ? "M-Pesa" : "E-Mola"}.
                      </p>
                    </div>
                  </div>
                </div>

                {/* Sec├º├úo de Finaliza├º├úo (Antes estava no card lateral) */}
                <div className="pt-4 space-y-4">
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-emerald-500">Envio autom├ítico ap├│s o pagamento</p>
                      <p className="text-[11px] text-emerald-500/70">O acesso ao seu produto ser├í enviado para o seu e-mail imediatamente.</p>
                    </div>
                  </div>
                  ...
                  >
                    {submitting ? (
                      ...
                      <div className="flex items-center justify-center gap-2">
                        <span>PAGAR AGORA ΓÇö {product.price.toLocaleString('pt-MZ', { minimumFractionDigits: 2 })} MT</span>
                  ...
                  <div className="flex flex-col items-center gap-2 text-muted-foreground">
                    <div className="flex items-center gap-1 text-xs font-bold uppercase tracking-widest opacity-60">
                      <ShieldCheck className="w-3 h-3" /> Pague com seguran├ºa via EnsinaPay
                    </div>
              ...
              <CardContent className="p-10 text-center space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-muted-foreground font-medium leading-relaxed">
                    Enviamos um pedido de pagamento para o n├║mero <span className="text-foreground font-bold">{form.payment_phone}</span>. 
                    Por favor, introduza o seu PIN no telem├│vel para confirmar.
                  </p>
                </div>
